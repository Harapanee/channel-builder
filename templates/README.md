...
- **画風(色・線・書体)もチャンネル可変**: すべて `src/scenes/style.ts` が唯一の切替点。値を差し替えるだけで、それを参照する全コンポーネントが自動追従する(個別のシーンに手を入れない)
  - `PALETTE` — 配色。キー名(paper/ink/…)は据え置き、値だけを差し替える
  - `LINE_STYLE` — 線の様式。`"rough"`(手描き画風=既定。ジッタ+Catmull-Rom平滑化)/ `"clean"`(均一で太い線=直線と正確な円弧)。`doodle-svg.ts` の `rough*Path` 群と `motion` の `boiling()` が参照する
  - `DOODLE_FONT` — 書体。`assets/fonts/` に同梱したTTFを指す(既定は手描き風の Yusei Magic)。**`family` にはTTFの内部ファミリー名を書く**(配布サイトの表示名ではない)。ライセンスは `assets/fonts/LICENSE.md` に記録すること
  - チャンネルの画風は `channel/bible.md` §8 が教義として定め、`DESIGN.md`(design-forge契約)が具体値を記録し、`style.ts` がそれを実装する — という3層構造
  - ⚠️ `LINE_STYLE: "clean"` は手描きの `boiling`(コマ単位の揺れ)を「滑らかな微小ドリフト」へ置き換える。**これはQAの静止検出(frozen_video)対策を兼ねている**ため、独自に無効化しないこと
...
npx tsx src/pipeline/probe-frames.ts episodes/<ep> --at 6,22,356 -o <出力先>  # 実フレーム取得(HFはこちら。1回5分未満=最大8枚)
npm run snapshot -- . --at 6,22,356 -o <出力先>  # hyperframes CLI版(完成尺では動かない。短いcomposition専用)
