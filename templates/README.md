...
また `npm run validate` は全チャンネル共通で**必須props検査(Rule 10)**を行う: コンポーネント別の必須props欠落(例: ComparisonSplit の left/right/mode)・絵コンテ残骸 `placeholderAssets`・Outro の channelName 欠落を検出する(プレースホルダpropsのままレンダーへ進む事故の防止)。
