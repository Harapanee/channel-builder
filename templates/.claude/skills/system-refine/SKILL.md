...
3. **テンプレートへ同期**(`~/.claude/skills/channel-builder/templates/`):
   - **IDENTICAL区分のファイル**(パイプライン・スキーマ・共有コンポーネント等): SRCからそのままコピー
...
4. **同期チェッカーで機械検証**: `node scripts/check-template-sync.mjs` が**全緑になるまで**終わらない。コード変更時はテンプレのコンパイル確認も推奨(スモーク環境で `tsc --noEmit`)
5. **ドキュメント同期(必須)**: 変更がユーザーから見える挙動(スキル・コマンド・エージェント・契約)を追加・変更した場合、(a) `README.md` の該当節(パイプライン図/ファイルの地図/エージェント一覧/コマンド早見表)を更新し、(b) `CLAUDE.md` の Key commands / Skills / Agents 節を更新する。README.md は IDENTICAL 区分のためテンプレートへも同一内容を同期する。**ドキュメント更新のない system-refine の完了宣言を禁止する**(ユーザー可視の変更が無い場合は「ドキュメント影響なし」と CHANGELOG に明記)
6. **記録**: CHANGELOG.md に「契機 / 変更 / 期待効果」+ **「全チャンネル適用」の明記**。git commit
7. **references更新**: 変更が channel-builder の手順に影響する場合(工程の追加・削除)、`~/.claude/skills/channel-builder/SKILL.md` と references/ も同時に更新する
