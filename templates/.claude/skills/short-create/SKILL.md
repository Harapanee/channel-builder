---
name: short-create
description: 完成済みエピソードの要素からショート動画(縦型1080x1920)を生成する。「/short-create ep001-xxx rank3-reasons」のように起動(フォーマットが1つなら第2引数省略可、引数なしはエピソード選択から)。台本→承認→TTS→ショット実装→Studio確認→公開メタデータ→夜間レンダーキュー投入までを実行する。
---
