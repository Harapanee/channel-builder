...
- **サムネは画面内の語だけで意味が完結する**(原則8-a)。**本編を一秒も見ていない人が読む面**であり、本編の要約ではない。見出し・吹き出しの語を1つずつ「画面内の情報だけで意味が取れるか」で検査し、取れない語は画面内で説明するか捨てる。特に**説明のない数値**(「四十日」「三千人」)と**本編の固有名**が危ない。数値を使うなら、何の数値かを同じ画面内に書くか絵で示す
- **サムネの切り口は動画の冒頭(最初の60秒)が扱う対象と一致させる**(原則8-b)。本編のどこかが回収する切り口でも、冒頭が別の話を始めるならクリック直後に期待と画面が食い違って離脱する。**中盤の傍流の挿話をサムネの主題にしない**
...
- [ ] **各案の見出し・吹き出しの語をすべて数え上げ、本編未視聴者が画面内の情報だけで意味を取れることを確認した**(内輪ワード・説明のない数値がゼロ。同 原則8-a)
- [ ] **3案の切り口が、動画の冒頭60秒が扱う対象と一致している**(中盤の傍流の挿話を主題にしていない。同 原則8-b)
