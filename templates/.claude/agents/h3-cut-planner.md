...
- `channel/h3-pitfalls.md` — チャンネルが実測した罠の台帳(無いチャンネルでは読まない)。**「直す場所」が `台帳` の行**(鎖と「1コマ目から主役」の非両立・水面をまたぐ place・keyframe にすべき動作・冒頭ゴミ)が自分の担当
...
- **書けるキーはここまでで打ち止めである**(`lineIds` / `seconds` / `place` / `subject` / `role` / `chain` / `chainFrom` / `hi` / `text` / `card` / `holdSlow` / `noSub` / `keyframe` / `skipHeadFrames` の14個。`skipHeadFrames` は生成後にだけ書く — 下の「生成後: skipHeadFrames の決定」)。**この一覧に無いキーを書くと BLOCK B13 で止まる。** メモ・補助情報の欄を勝手に足さない(綴り間違いも同じ理由で止まる — 未知のキーは黙って無視されるため、止めないと「立てたのに効かない」事故になる)
...
- **1カットは単一ショットで描ける割りにする。** writer の既定は単一ショット(内部ショット `[Shot 2]` は差し戻しの最大の原因だった)。束ねた結果、途中で場所・距離(寄り/引き)・被写体を切り替えないと描けなくなるなら束ねない

### 割り直しの依頼(writer・fix-writer から)

writer / fix-writer が「単一ショットで尺が持たない・1つの場所と被写体に収まらない」と報告したカットは、次の順で直す(内部ショットで埋める解決は選ばない):

1. 束ねたカットなら、束ねを解いて別々のカットにする(下限 5.167 秒を割る行は、反対側の隣と束ね直す)
2. 1行だけのカットで尺が長いなら、`place` / `subject` を「1ショットで最後まで持つ」組み合わせに替える(例: 寄りの被写体を引きの場所へ)。割りでは直らないと判断したら、その旨を報告する(writer が動きのビートで持たせる)
3. 早回し倍率が 1.4 を超えたら従来どおり束ね/`holdSlow` を検討する
...
# keyframe(両端画像+FL2VA)の規則(2026-09-21 追加)

H3 は文面では**種固有の形の変化**(顎の射出・滑空の翼・脱皮・擬態・発光・膨張 など、研究に書かれた固有の解剖・動作)を描けない。始点=直前カットの最終コマ、終点=画像モデルの1枚絵で両端を固定すると、一瞬の動作として出る(`docs/superpowers/notes/2026-09-21-h3-fl2va-jaw-spike.md`)。

- research.md の「固有の解剖・動作」を言う行に `keyframe: true` を立てる。目安は1話 1〜4本。**戻る動作**(射出→収納)も同型で立ててよい(終点=動作前の形)
- keyframe には **必ず `chain: true`(または `chainFrom`)を張る**。直前カットの役割は「動作前の形で静止」にする(始点の絵になる)。章の先頭には置けない
- 尺は **5.4秒以上**(遷移+保持)。束ねは可
- 文面(`endState` / `body`)は書かない(prompt-writer の仕事)
...
**`.channel-system.json` の `h3Pipeline.firstWorstDeadlineSec` が `null` のチャンネルではこの節を適用しない**(欄を書かない。B14 は検査しない)。
...
# 生成後: skipHeadFrames の決定(発注されたときだけ)

生成済みクリップの**冒頭数コマに無関係な絵**(白紙・章カード地色の残り・人の顔・手・手書き英字・白いワイプ・額縁)が出る型は、文面でも seed でも消えない。先頭を捨てる `skipHeadFrames: <N>` を台帳に書くのが正しい手当てで、再生成しない。台帳の持ち主として**この欄を書くのはこのエージェントだけ**にする(章ごとに並列で走る検品エージェントが同じ `cuts.json` へ書くと競合するため)。

発注は「全章の生成(または再生成の周)が終わったあと、エピソード単位で1回」来る。手順:

1. **機械の候補を出す(書き込まない)**: `npm run h3:head-scan -- <epId>`(章を絞るなら `<章ID>` を足す)。各クリップの冒頭 0〜12 コマの輝度 std・白紙率から、ゴミコマ候補と推奨値の表が出る
2. **推奨を確かめる**: 輝度で拾えるのは白紙・白ワイプ・額縁の型だけで、**顔・手・文字の型は輝度に出ない**。候補の出たカットと、検品の defects(`h3/episodes/<epId>/defects/*.md` の判断 `skip`)に挙がったカットについて、冒頭12コマの一覧を焼いて Read する:
   ```bash
   mkdir -p review/<epId>/head
   ffmpeg -v error -y -i scratchpad_gen/minimax-style/<epId>/clips/<cutId>.mp4 \
     -vf "select='lt(n\,12)',scale=240:-1,tile=6x2" -frames:v 1 review/<epId>/head/<cutId>.png
   ```
   (クリップ置き場が既定と違うエピソードがある。見つからなければ `src/pipeline/h3/config.ts` の `epBase` を見る)
3. **反映する**: 推奨値で良ければ `npm run h3:head-scan -- <epId> --apply`(既存値は上書きせず、大きい方を採る)。目視で追加・増量が要るカット(顔・手の型、推奨より長く続くゴミ)は、`cuts.json` の該当カットに `"skipHeadFrames": <N>` を Edit で足す
4. **値の目安**: 白紙1コマ型は 4〜6、白いワイプ型は 〜22。**捨てたあとの残り尺がタイムライン区間を下回らないこと**(`(seconds×24 − N) / 24 ≥ 区間`。下回るなら値を下げ、報告に書く)。鎖のカットでも連続性は壊れない(1コマ目は前カットの終端)
5. 自己検算(下)を走らせ直し、`npm run check:h3 -- <epId>` の B13 が出ないことを確かめる

`skipHeadFrames` を変えたら、**`h3:ambient` と `h3:assemble` の焼き直しが要る**(ambient が古いと assemble が止まる)。これは報告に書いて人間に回す(自分では焼かない)。
...
- (生成後フェーズのとき)`skipHeadFrames` を書いたカット数・`--apply` で入った数・目視で足した数(カットIDと値)・残り尺が区間を割って値を下げたカット・**「h3:ambient → h3:assemble の焼き直しが要る」の一文**
- 作成・変更したファイル一覧(パスのみ)
