...
**生成物・調達物を1枚ずつ Read しない。** 画像はコンテキストに残り続け、以後の全ターンで
毎回課金される。2026-08-02 の sekaishi-longform ep002 実測では、1本のエージェントが
19枚(8.74MB)を個別に Read し、それを110ターン持ち回っていた。

手順は次のとおり:

1. 1件生成するごとに `library.json` へ登録する(まとめて最後に登録しない)。
   中断されても再開時に既登録ぶんを飛ばせるようにするため
2. **セット全体が揃ってから**、コンタクトシート(横並びJPG)を1枚だけ作る:
   `magick montage <生成物...> -tile 4x -geometry 320x320+8+8 -background '#222' <出力>.jpg`
3. **そのコンタクトシートを1回だけ Read して**、全件の顔文法一致・画風・シルエット可読性を
   まとめて自己評価する
4. 個別 Read が許されるのは、コンタクトシートで問題が見えた **その1枚を作り直すとき**だけ

除去率・寸法・塗り検査など**数値で判定できるものは画像を読まずコマンドで確認する**
(`identify -format "%wx%h"` / remove-bg の出力 / 緑面積の自動検出)。

**再開時の重複防止**: 着手前に `assets/library.json` を読み、担当ぶんのうち登録済みの
assetId は生成しない。枠切れ・中断からの再開でセット全体をやり直すと、実測で
1回あたり23分・約10Mトークンを捨てることになる(ep002 の素材工程で発生)。
