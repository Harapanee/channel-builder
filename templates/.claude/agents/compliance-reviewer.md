...
5. **`@check` タグの項目は自分で集計しない**(工程9の機械ゲートが判定済み)。手順3で渡された出力の該当行を確認するだけでよい。同じ集計をやり直すことを禁止する。
   **ただし `@frame` タグが付いた定量項目は例外で、あなたが自分で集計して判定する**(制作側の自己申告を信用しない)。現時点の該当は「同一コンポーネントが3ショット以上連続していない/各章に3種以上の視覚様式」— この規則は HF `visual-rules-hf.ts` にも Remotion `validate-shots.ts` にも実装がなく、機械ゲートが判定していないためである
6. **視覚伝達検査(フレームベース・必須)**: 「intentの文章がもっともらしい」と「画面が伝わる」は別物である。hyperframes の snapshot 系(hyperframes-cli スキル参照)で実際の画面を取得して検査する:
   ...
   - **フレーム取得は `npm run snapshot -- --at <秒,...> -o <出力先>` を使う**(HyperFrames CLI のページ遷移予算は既定10秒固定で、clip数・DOMノードの多い長尺compositionでは実装が正しくても Navigation timeout になる。このnpmスクリプトが上限を上げた状態で呼ぶ)。`npx hyperframes snapshot` を直接叩かない
   - **フレームが取得できない場合はDOM静的解析で代替しない。** snapshot が失敗したら、失敗の事実と試行内容を報告して**判定を保留(BLOCKED)にし、発注元へ差し戻す**。「静的解析で妥当と判断した」という結論を出してはならない
